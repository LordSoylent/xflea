...
#if defined(NDEBUG)
#error "zXflea cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSub spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSub spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSub spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 3 * COIN;
    } else if (nHeight < 341000) {
        nSubsidy = 166 * COIN;
    } else if (nHeight < 541000) {
        nSubsidy = 161 * COIN;
    } else if (nHeight < 741000) {
        nSubsidy = 156 * COIN;
    } else if (nHeight < 941000) {
        nSubsidy = 151 * COIN;
    } else if (nHeight < 1141000) {
        nSubsidy = 146 * COIN;
    } else if (nHeight < 1341000) {
        nSubsidy = 141 * COIN;
    } else if (nHeight < 1541000) {
        nSubsidy = 136 * COIN;
    } else if (nHeight < 1741000) {
        nSubsidy = 131 * COIN;
    } else if (nHeight < 1941000) {
        nSubsidy = 126 * COIN;
    } else if (nHeight < 2141000) {
        nSubsidy = 121 * COIN;
    } else if (nHeight < 2341000) {
        nSubsidy = 116 * COIN;
    } else {
        nSubsidy = 111 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	ret = 0;
        ...
        ret = blockValue * 0.937;
    } else if (nHeight < 541000) {
        ret = blockValue * 0.931;
    } else if (nHeight < 741000) {
        ret = blockValue * 0.929;
    } else if (nHeight < 941000) {
        ret = blockValue * 0.927;
    } else if (nHeight < 1141000) {
        ret = blockValue * 0.924;
    } else if (nHeight < 1341000) {
        ret = blockValue * 0.921;
    } else if (nHeight < 1541000) {
        ret = blockValue * 0.919;
    } else if (nHeight < 1741000) {
        ret = blockValue * 0.916;
    } else if (nHeight < 1941000) {
        ret = blockValue * 0.912;
    } else if (nHeight < 2141000) {
        ret = blockValue * 0.909;
    } else if (nHeight < 2341000) {
        ret = blockValue * 0.905;
    } else {
        ret = blockValue * 0.9;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from zXflea
...
{
    RenameThread("zxflea-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZXFLEAMinted()
{
...
void RecalculateZXFLEASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zXFLEA supply
...
bool RecalculateXFLEASupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSub with serial %s is already in the block %d\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert XFLEA to zXFLEA
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSub spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSub serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSub mints and %d zSub spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // zXflea: We use certain sporks during IBD, so check to see if they are
